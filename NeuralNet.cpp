...
CNeuralNet::CNeuralNet(int nInput, int nOutput, int nHiddenLyrNeurons, double LearnRate, int MaxEpoch, double ErrorTolerance)
{
	m_nHiddenLayer = 1; //暂时只支持一个隐藏层的网络
	m_nInput = nInput;
	m_nOutput = nOutput;
	m_nNeuronsPerLyr = nHiddenLyrNeurons;
	m_dLearningRate = LearnRate;
	m_nMaxEpoch = MaxEpoch;
	m_dMinError = ErrorTolerance;
...
bool CNeuralNet::Train(vector<iovector>& SetIn, vector<iovector>& SetOut)
{
	...
	do
	{
		...
		{
			strOutMsg.Format(_T("训练在第%d个时代出现错误！"), GetEpoch());
...
int CNeuralNet::Recognize(const vector<double> & InputSample,double &dConfidence)
{
	int nBestMatch; //类别标号
	...
	if (!CalculateOutput(InputSample, outputs))
	{
		AfxMessageBox(_T("Recognize failed"));
		return -1;
	}
...
//保存训练结果
bool CNeuralNet::SaveToFile(LPCTSTR lpszFileName, bool bCreate)
{
	CFile file;
	if (bCreate) //新建模式
	{
...
bool CNeuralNet::LoadFromFile(LPCTSTR lpszFileName, DWORD dwStartPos)
{
	CFile file;
	...
	if (m_nInput != header.m_nInput
		...
		return false;
	*/
	m_nInput = header.m_nInput;
	m_nNeuronsPerLyr = header.m_nNeuronsPerLyr;
	m_nOutput = header.m_nOutput;

	CreateNetwork();   //根据以上信息（输入层单元数，隐藏层单元数，输出层单元数）创建神经元网络对象
